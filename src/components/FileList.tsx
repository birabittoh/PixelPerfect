...
export const FileList: React.FC<FileListProps> = ({ files, onDownload, onDelete, onCropRequest }) => {
  ...
  const sources = files.filter(f => f.status === 'pending');
  const exports = files.filter(f => f.status !== 'pending');
  ...
  return (
    <div className="space-y-6">
      {sources.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 overflow-hidden">
          <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/50">
            <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Source Images</h3>
          </div>
      ...
      {exports.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 overflow-hidden">
          <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/50">
            <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Exports</h3>
          </div>
